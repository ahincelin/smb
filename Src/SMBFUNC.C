...
int getinputs()
{
  ...
  char *temp = NULL,ch,levelfile[13],graphfile[13];
  char far *tempbm;
  ...
  if ((in=fopen(levelfile,"rb"))==NULL)
  {
    exitgame();
    printf("%s could not be found!\n",levelfile);
    exit(1);
  }
  data.maxlength=(int)fgetc_no_cr(in);
  yinit=(int)fgetc_no_cr(in);
  data.startpos[0]=(int)fgetc_no_cr(in);
  data.startpos[1]=(int)fgetc_no_cr(in);
  blocknum=(int)fgetc_no_cr(in);
  backnum=(int)fgetc_no_cr(in);
  fseek(in,0,SEEK_SET);
...
#else
	x_setpalette(index, color.red, color.green, color.blue);
#endif
}
...
void drawscreen(int x)
{
  ...
  for (loop1=1;loop1<=13;loop1++)
	...
	{
	  dy+=data.bumploop;
	  x_put_masked_pbm((loop2<<4)-x+32,(loop1<<4)+dy,HiddenPageOffs, (unsigned char *) data.block[data.world[loop1][loop2]-1]);
	  data.bumploop++;
	  if(data.bumploop==3)
	  {
	    data.bumploop=0;
	  ...
	  x_put_masked_pbm((loop2<<4)-x+32,(loop1<<4),HiddenPageOffs, (unsigned char *) data.block[data.world[loop1][loop2]-1]);
      }
  drawcoin(x);
  drawscore(x);
...
void drawmario(int x, int y)
{
  ...
  for (loop1=0;loop1<=15;loop1++)
    for (loop2=0;loop2<=15;loop2++)
      if ((signed char)data.mario[abs(data.sprite/2)][loop1][loop2]>=0)
...
#else
	x_graph_char(xc, yc, c, color, offset);
#endif
}
...
int startscreen()
{
  ...
  int y=191;
  //int choice;
  int screenx,offset,numplayers=1;
  FILE *logoin;
  ...
  x_page_flip(0,0);
  xlib_batch_end();
  os_notify("Waiting for player selection");
...
void drawcoin(int x)
{
  ...
  for (loop=0;loop<num;loop++)
  {
    ...
    if (((c->coinx)*16+32-x)>16)
      x_put_masked_pbm((c->coinx)*16+32-x,(c->coiny)*16+c->dy,HiddenPageOffs, (unsigned char *) data.coin[abs((c->coinloop)%3)]);
    c->coinloop++;
    c->dy+=c->coinloop;
    if (c->coinloop>=10)
       removecoin();
